...
class GNNModel(nn.Module):
    ...
    def forward(self, x, edge_index, batch=None, center_idx=None):
        ...
        gesammelte_nachrichten = self.classifier(combined_features)

        return gesammelte_nachrichten
...
class InduCEExplainer:
    ...
    def get_n_hop_neighborhood(self, G, center_node, n_hops=2):
        ...
        neighborhood_list = [center_node] + [n for n in linju if n != center_node]
    ...
    def generate_counterfactual(self, x, edge_index, center_idx, max_iterations=200,
        ...
        n_hops = 2
        linju = self.get_n_hop_neighborhood(G, center_idx.item(), n_hops=n_hops)

        edges_modified = 0
        current_pred = orig_pred
        current_fraud_prob = orig_fraud_prob

        possible_deletions = [(u, v) for u, v in unique_edges if u in linju or v in linju]

        possible_additions = []
        for node in linju:
            for neighbor in linju:
        ...
        while edges_modified < edge_budget and current_pred == 1:
            ...
            for edge in possible_deletions:
                recompense = self.compute_edge_reward(cf_x, cf_edge_index, center_idx, edge, False, current_fraud_prob)
                edge_rewards[('deletion', edge)] = recompense

            for edge in possible_additions:
                recompense = self.compute_edge_reward(cf_x, cf_edge_index, center_idx, edge, True, current_fraud_prob)
                edge_rewards[('addition', edge)] = recompense
        ...
        if current_pred == 1:
            perturbations = {}
            for idx, node_idx in enumerate(linju):
        ...
        return tonggye['success'], cf_edge_index, cf_x, tonggye
...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, x, edge_index, actions):
        ...
        for action_type, (src, dst) in actions:
            ...
            tensuu = self.mlp(combined)
            action_tensuus.append(tensuu.item())

        return torch.tensor(action_tensuus, device=self.device)
...
def run_explanation_experiment(dataset_name, model_type, num_samples=50, device='cuda'):
    ...
    for sample_key in tqdm(sample_keys, desc="Generating counterfactuals"):
        ...
        if pred == 1:
            ...
            success, cf_edge_index, cf_x, tonggye = explainer.generate_counterfactual(
                x, edge_index, center_idx,
                max_iterations=150,
                edge_budget=5
            )

            results.append(tonggye)
...
def visualize_counterfactual(sample, cf_edge_index, cf_x, tonggye, dataset_name, model_type, sample_id):
    ...
    removed_edges = tonggye['removed_edges']

    added_edges = tonggye['added_edges']

    modified_nodes = tonggye['modified_nodes']
    ...
    plt.title(f"Original Graph (Prediction: Fraud, P={tonggye['original_fraud_prob']:.2f})")
    plt.axis('off')
    ...
    plt.title(f"Counterfactual Graph (Prediction: Non-Fraud, P={tonggye['final_fraud_prob']:.2f})\n"
              f"{tonggye['edges_removed']} edges removed, {tonggye['edges_added']} edges added, "
              f"{tonggye['nodes_modified']} nodes modified")
    plt.axis('off')
