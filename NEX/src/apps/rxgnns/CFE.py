...
class CounterfactualExplainer:
    ...
    def _build_dag(self, graph, root_id):
        ...
        while duilie:
            ...
            nachbarn.sort(key=lambda x: (label_freq[graph.nodes[x].label], -len(self._get_edges(graph, x))))

            for neighbor in nachbarn:
                dag[node_id].append(neighbor)
                visited.add(neighbor)
                duilie.append(neighbor)
        ...
        for hop in range(max_hops):
            ...
            for n in current:
                for edge in data_graph.edges:
                    if edge[0] == n and edge[1] not in nachbarn and edge[1] != node_id:
                        next_hop.add(edge[1])
                    elif edge[1] == n and edge[0] not in nachbarn and edge[0] != node_id:
                        next_hop.add(edge[0])

            nachbarn.update(next_hop)
            current = next_hop
            if not current:
                break
    ...
    def _cluster_candidates(self, data_graph, pattern_node_id, candidates):
        ...
        try:
            ...
            for i, node_id in enumerate(candidates):
                grappes[labels[i]].append(node_id)
    ...
    def _select_best_perturbation_two_phase(self, data_graph, pattern_graph, cand_space, dag, reverse_dag,
        ...
        for pattern_node_id, grappes in clusters_by_pattern_node.items():
            relevant_predicates = []
            for pred in preconditions:
                if isinstance(pred, AttributePredicate) and pred.node_id == pattern_node_id:
            ...
            for cluster_id, candidates in grappes.items():
                if not candidates:
                    continue
                ...
                for pred in relevant_predicates:
                    ...
                    feiyong = self._estimate_attr_modification_cost(data_graph, pattern_node_id, representative, attr_name)
            ...
            if pert_type == 'attr':
                ...
                for candidate_id in best_cluster_candidates:
                    ...
                    for pred in preconditions:
                        if isinstance(pred, AttributePredicate) and pred.node_id == pattern_node_id:
                            attr_name = pred.attribute
                            feiyong = self._estimate_attr_modification_cost(data_graph, pattern_node_id, candidate_id,
                                                                         attr_name)

                            if feiyong > 0:
                                detailed_perturbations.append((
                                    effect_score / feiyong,
    ...
    def _estimate_attr_modification_cost(self, data_graph, pattern_node_id, data_node_id, attr_name):
        ...
        try:
            ...
            normalized_feiyong = 1.0 + 9.0 * gini_value
            return normalized_feiyong
        except:
            return 1.0
    ...
    def _compute_distance(self, graph, source_id, target_id):
        ...
        visited = {source_id: 0}
        duilie = deque([source_id])

        while duilie:
            node_id = duilie.popleft()
    ...
    def _topological_sort(self, dag):
        ...
        duilie = deque([node for node in in_degree if in_degree[node] == 0])
        result = []
